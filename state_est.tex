...
\end{align}%
%
Since it will be useful later, let us also write down the approximation to a 
conditional distribution that can be easily derived from equation~\eqref{eq:joint_predictive_approximation}, 
namely:
\begin{align}
	\Pdf{\xkk}{\xk,\y_{1:k-1}}&\approx \N[\xkk]{\m'_{k-1}}{\P'_{k-1}}\\
	\v{G}_k &= \v{C}_{k^-}\v{P}_{k|k-1}^{-1}
...
\eqref{eq:smooth_joint_bayes} and the final steps are given
by some Gaussian manipulations. The smoothing recursions
\begin{align}
\begin{split}
	\Pdf{\xk,\x_{k+1}}{\y_{1:T}}&=\Pdf{\xk}{\x_{k+1},\y_{1:k}}\Pdf{\x_{k+1}}{\y_{1:T}}\\
	&\approx
	\N{
	\begin{bmatrix}
		\m_{k-1|T}\\
