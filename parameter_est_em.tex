...
that it is taken with respect to the posterior distribution of the states with parameter $\Th'$, 
that is $\post{\Th'}$. 
...
as compared to MCMC methods. Unfortunately it seems that applying
...
Equation~\eqref{eq:EM_M_A} and for $\v{H}$ from Equation~\eqref{eq:EM_M_H}. Also for evaluating the convergence,
