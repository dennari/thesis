...
\begin{example}[Maximum of a mixture of Gaussians with BFGS]
\end{example}
\missingfigure{Show the progress of BGFS optimization for the local mode of a mixture of
...
of Equation~\eqref{eq:spherical} will be applied to integrals intractable in closed form.
...
\end{split}
\end{align}
Let us denote the sigma points needed in the integral for $\S$ by 
$\sig_{k|k-1}=\m_{k|k-1}+\sqrt{\P_{k|k-1}}\usig$,
where $j=1,\dots,2\,d_x$. For unclutteredness, let us also denote the constant
weight by $w\equiv \sfrac{1}{2d_x}$.
...
so that 
\begin{align}
	\dpd{\S}{\theta_i} \approx \quad{}w \sum_{j=1}^{2\,d_x}\bigg[
	 &\Big(\dpd{\hhi[\big]{\sig_{k|k-1}}}{\theta_i}-\dpd{\ym}{\theta_i}\Big)\Big(\hhi[\big]{\sig_{k|k-1}}-\ym\Big)^\tr+\\
	 &\Big(\hhi[\big]{\sig_{k|k-1}}-\ym\Big)\Big(\dpd{\hhi[\big]{\sig_{k|k-1}}}{\theta_i}-\dpd{\ym}{\theta_i}\Big)^\tr\bigg]
	+\dpd{\v{R}}{\theta_i}.
	\label{eq:dS_cubature_approx}
\end{align}
%
The approximate partial derivative of $\ym$ is easily derived from Equation~\eqref{eq:update_mean_intergral}:
\begin{align}
	\dpd{\ym}{\theta_i} &\approx{} w\sum_{j=1}^{2\,d_x}\dpd{\hhi[\big]{\sig_{k|k-1}}}{\theta_i}.
	\label{eq:dmu_cubature_approx}
\end{align}
%
The approximate partial derivative of $\h$ evaluated at the sigma points $\sig_{k|k-1}$ that themselves 
depend on $\Th$ is given by
\begin{align}
	\dpd{\hhi[\big]{\sig_{k|k-1}}}{\theta_i} \approx 
	\jacob{\h}{\sig_{k|k-1}}\dpd{\sig_{k|k-1}}{\theta_i},
	\label{eq:dh_cubature_approx}
\end{align}
where $\jacob[\displaysize]{\h}{\cdot}$ is the Jacobian of $\h$.
%
To find $\pd{\sig_{k|k-1}}{\theta_i}\equiv \pd{\m_{k-1|k-1}}{\theta_i}+\pd{\sqrt{\P_{k-1|k-1}}}{\theta_i}\usig$, let us write the the approximations
\begin{align}
	\m_{k|k-1} &\approx{} w \sum_{j=1}^{2\,d_x} \ffi[\big]{\sig_{k-1|k-1}} \\
	\P_{k|k-1} &\approx{} w \sum_{j=1}^{2\,d_x} \Big(\ffi[\big]{\sig_{k-1|k-1}}-\m_{k-1|k-1}\Big)\Big(\ffi[\big]{\sig_{k-1|k-1}}-\m_{k-1|k-1}\Big)^\tr,
	\label{tablelabel}
\end{align}
where we have again denoted $\sig_{k-1|k-1}\equiv{}\m_{k-1|k-1}+\sqrt{\P_{k-1|k-1}}\usig$.
Analogously to Equations \eqref{eq:dmu_cubature_approx} and \eqref{eq:dh_cubature_approx} we can write
\begin{align}
	\dpd{\m_{k|k-1}}{\theta_i} &\approx{} w\sum_{j=1}^{2\,d_x}%
	\jacob{\f}{\sig_{k-1|k-1}}\dpd{\sig_{k-1|k-1}}{\theta_i}.
	\label{eq:dmpred_cubature_approx}
\end{align}
The partial derivative of the Cholesky decomposition of a matrix be obtained for example by differentiating
an algorithm for computing a the decomposition. The partial derivative of the matrix itself needs to be
computed separately. Analogously to Equation~\eqref{eq:dS_cubature_approx} we get
\begin{align}
	\dpd{\P_{k|k-1}}{\theta_i} \approx{} w\sum_{j=1}^{2\,d_x}\bigg[
	&\Big(\dpd{\ffi[\big]{\sig_{k-1|k-1}}}{\theta_i}-\dpd{\m_{k|k-1}}{\theta_i}\Big)\Big(\ffi[\big]{\sig_{k-1|k-1}}-\m_{k|k-1}\Big)^\tr+\\
	&\Big(\ffi[\big]{\sig_{k-1|k-1}}-\m_{k|k-1}\Big)\Big(\dpd{\ffi[\big]{\sig_{k-1|k-1}}}{\theta_i}-\dpd{\m_{k|k-1}}{\theta_i}\Big)^\tr\bigg]
	\label{eq:dPpred_cubature_approx}
\end{align}

\begin{align}
	\dpd{\m_{k|k}}{\theta_i} &=  \dpd{\m_{k|k-1}}{\theta_i}+\dpd{\v{K}_{k}}{\theta_i}\yk
	\dpd{\v{K}_{k}}{\theta_i}\ym+\v{K}_{k}\dpd{\ym}{\theta_i}.
	\label{eq:dm_cubature_approx}
\end{align}
...
\begin{align}
	...
	&\Big(\sig_{k|k-1}-\m_{k|k-1}\Big)\Big(\dpd{\hhi[\big]{\sig_{k|k-1}}}{\theta_i}-\dpd{\ym}{\theta_i}\Big)^\tr\bigg]
