...
they commonly require the gradient of the objective function to be available \todo{Get some references for nonlinear programming}.
...
indexed by the parameter $\gv{\psi}$, $\tPX$, over the states.
...
Let us denote the \emph{point-to-set} map implicitly defined by the EM
algorithm by 
\begin{align}
...
\end{align} 
We can then arrive at the following expression for $\dif\EMM{\Th_\star}$ \parencite{Elliott1999,Lange1995}:
\begin{align}
	\dif\EMM{\Th_\star} &= \v{I}-\dQ^{-1}\dL.
\end{align}
also writable as
\begin{align}
	\dpd{\EMM{\Th_\star}}{\Th} &=\v{I}-\E{\dpd{\log\cLH{\Th_\star}}{\Th}}^{-1}_{\post{\Th_\star}}
	\E{\dpd{\log\LH{\Th_\star}}{\Th}}_{\post{\Th_\star}},
\end{align}
where the latter term is commonly interpreted as the proportion of ``measured'' information compared to
all information.
