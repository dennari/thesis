...
\subsubsection{Maximuma a posteriori and maximum likelihood}
...
\begin{align}
\begin{split}
	\EMQ{\Th'}{\Th} =&-\frac{1}{2}\Tr{\gv{\Sigma}_0^{-1}\E{\left(\x_0-\gv{\mu}_0\right)\left(\x_0-\gv{\mu}_0\right)^\tr}}-\frac{1}{2}\log\detr{\gv{\Sigma}_0}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{Q}^{-1}\sum_{k=1}^T\E{\left(\x_k-\v{A}\xkk\right)\left(\x_k-\v{A}\xkk\right)^\tr}}-\frac{T}{2}\log\detr{\v{Q}}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{R}^{-1}\sum_{k=1}^T\E{\left(\y_k-\v{H}\xk\right)\left(\y_k-\v{H}\xk\right)^\tr}}-\frac{T}{2}\log\detr{\v{R}}
\end{split}
\label{eq:eclLH}
\end{align} 
...
$\II{2}$ from equation~\eqref{eq:LGSSM_I2} and $\II{3}$\todo{index $\v{I}$:s} from equation~\eqref{eq:LGSSM_I3}.
...
we shall utilize the BFGS nonlinear optimization method to
maximize the intermediate quantity with respect to $\Th$. This however
leads to the requirement of being able to compute 
...
\label{eq:EMQ_approx}
\begin{split}
	\EMQ{\Th'}{\Th} \approx& -\frac{1}{2}\Tr{\gv{\Sigma}_0^{-1}\II{1}}-\frac{1}{2}\log\detr{\gv{\Sigma}_0}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{Q}^{-1}\II{2}}-\frac{T}{2}\log\detr{\v{Q}}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{R}^{-1}\II{3}}-\frac{T}{2}\log\detr{\v{R}},
\end{split}
\end{align}
where $\II{1}$ is the same as in \eqref{eq:I1_general} and
\begin{align}
\II{2} &= 
\sum_{k=1}^T\iint_{\mathcal{X}}\left(\xk-\ff\right)\left(\xk-\ff\right)^\tr \nonumber\\
&\quad\times\Pdf{\xk,\xkk}{\Y,\Th'}\,\dif\xkk\dif\xk \label{eq:I2_general}\\
&\approx \sum_{k=1}^T\iint_\mathcal{X}\left(\x_k-\f_{k-1}\right)\left(\x_k-\f_{k-1}\right)^\tr \nonumber\\
...
\bm{\v{I}\\-\v{I}}\nonumber\\	
\shortintertext{and}
\II{3} &= \sum_{k=1}^T\defint{\mathcal{X}}{}{\left(\yk-\hh\right)\left(\yk-\hh\right)^\tr\Pdf{\xk}{\Y,\Th'}}{\xk}\\
&\approx \sum_{k=1}^T\defint{\mathcal{X}}{}{\left(\y_k-\h_{k}\right)\left(\y_k-\h_{k}\right)^\tr\N[\xk]{\m_{k|T}}{\P_{k|T}}}{\xk} \label{eq:I3_approx}\\
&=
\bm{\v{I} \\ -\v{I}}^\tr	
\sum_{k=1}^T
...
Let us find out the formal differential of a general log-Gaussian, where
...
\begin{align}
\begin{split}
	\ff &=\gv{\Phi}\gv{\rho}_1(\x)+\dots+\gv{\Phi}_m\gv{\rho}_m(\x)\\
	&=
	\begin{bmatrix}
...
Now that the main properties of the two algorithms have been presented,
at least two ideas of combining the methods come to mind: using gradient based
optimization by computing the score through the Fisher's identity \eqref{eq:EM_gradients}
and utilizing gradient based optimization in the M-step of the EM algorithm.
Both of these ideas have been presented in the literary. 

The first idea is used in the expectation-conjugate-gradient (ECG) 
method of \textcite{Salakhutdinov2003a}.
Furthermore, it is termed the ``easy gradient recipe" in \textcite{Olsson2007}. 
The gradient computations of equations
...
The second idea is analyzed in \parencite{Lange1995},
where a \emph{single} step of Newton's method is run on every M-step.
Using only a single step was only a heuristic argument in
\parencite{Lange1995}, which raises the question of the optimal
amount of increase that should be achieved in the M-step. 
