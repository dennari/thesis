...
\begin{align*}
\begin{split}
	\cLH =&-\frac{1}{2}\left(\x_0-\gv{\mu}_0\right)^\tr\gv{\Sigma}_0^{-1}\left(\x_0-\gv{\mu}_0\right)-\frac{1}{2}\log\det{\gv{\Sigma}_0}\\
	&-\frac{1}{2}\sum_{k=1}^T\left(\x_k-\v{f}(\x_{k-1})\right)^\tr\v{Q}^{-1}\left(\x_k-\v{f}(\x_{k-1})\right)-\frac{T}{2}\log\det{\v{Q}}\\
	&-\frac{1}{2}\sum_{k=1}^T\left(\y_k-\v{h}(\x_{k})\right)^\tr\v{R}^{-1}\left(\y_k-\v{h}(\x_{k})\right)-\frac{T}{2}\log\det{\v{R}}\\
	&+\mathtt{const}
\end{split}
\end{align*}
...
\begin{align}
\begin{split}
	\EMQ{\Th'}{\Th} =&-\frac{1}{2}\Tr{\gv{\Sigma}_0^{-1}\E{\left(\x_0-\gv{\mu}_0\right)\left(\x_0-\gv{\mu}_0\right)^\tr}}-\frac{1}{2}\log\det{\gv{\Sigma}_0}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{Q}^{-1}\sum_{k=1}^T\E{\left(\x_k-\f_{k-1}\right)\left(\x_k-\f_{k-1}\right)^\tr}}-\frac{T}{2}\log\det{\v{Q}}\\
	&-\frac{1}{2}\Tr[\bigg]{\v{R}^{-1}\sum_{k=1}^T\E{\left(\y_k-\h_{k}\right)\left(\y_k-\h_{k}\right)^\tr}}-\frac{T}{2}\log\det{\v{R}}
\end{split}
\label{eq:eclLH}
\end{align} 
...
The two methods we have discussed could clearly benefit from each other:
EM provides a seemingly simpler method for score computation than
the sensitivity equations and the M-step maximization could obviously
utilize a gradient based optimization algorithm. 

The first observation has lead to the development of the
expectation-conjugate-gradient (ECG) method of \textcite{Salakhutdinov2003a}
and the coining of the ``easy gradient recipe'' of \textcite{Olsson2007} 

The second observation has lead to the 
optimization in the M-step of EM. This is analyzed in \parencite{Lange1995},
...
Thus an obvious combination, explored for example in \textcite{Salakhutdinov2003a}
and \textcite{Olsson2007}, is to compute the gradient through Fisher's identity
and then use gradient based methods.  
