...
\begin{align*}
\begin{split}
	\cLH =&-\frac{1}{2}\left(\x_0-\gv{\mu}_0\right)^\tr\gv{\Sigma}_0^{-1}\left(\x-\gv{\mu}_0\right)-\frac{1}{2}\log\abbs{\gv{\Sigma}_0}\\
...
\end{split}
\end{align*}
Taking the expectation w.r.t. $\Pdf{\X}{\Y,\Th'}$ (assumed implicitly in the notation), applying the identity $\x^\tr\X\x=\Tr{\x^\tr\X\x}=\Tr{\X\x\x^\tr}$
...
\begin{align}
	...
	\v{I}_2 &= \sum_{k=1}^T\defint{\mathcal{X}\times 2}{}{\left(\x_k-\f_{k-1}\right)\left(\x_k-\f_{k-1}\right)^\tr\Pdf{\bm{\xk^\tr&\xkk^\tr}^\tr}{\Y,\Th'}}{\bm{\xk&\xkk}^\tr}\\
	\v{I}_3 &= \sum_{k=1}^T\defint{\mathcal{X}}{}{\left(\y_k-\h_{k}\right)\left(\y_k-\h_{k}\right)^\tr\Pdf{\xk}{\Y,\Th'}}{\xk}
\end{align}
...
\begin{align}
	\v{Q}_{j+1}&=\frac{1}{T}\sum_{k=1}^T\E{\left(\xk-\f_{k-1}\right)\left(\xk-\f_{k-1}\right)^\tr}
\end{align}
The derivation of the update equation for the next estimate of $\v{R}$ is exactly analogous, giving
\begin{align}
	\v{R}_{j+1}&=\frac{1}{T}\sum_{k=1}^T\E{\left(\yk-\h_{k}\right)\left(\yk-\h_{k}\right)^\tr}
\end{align} 
...
\sum_{k=1}^T\bm{\E{\xk\xk^\tr} & \E{\xk\xkk^\tr} \\ \E{\xkk\xk^\tr} & \E{\xkk\xkk^\tr} }
\bm{\v{I}\\-\v{A}^\tr}\\
\v{I}_{3}&=
\bm{\v{I} & -\v{H}}	
\sum_{k=1}^T
...
\begin{align}
	...
	\sum_{k=1}^T\E{\dpd{}{s_i}\left(\x_k-\v{A}\v{g}(\x_{k-1})\right)^\tr\v{Q}^{-1}\left(\x_k-\v{A}\v{g}(\x_{k-1})\right)}{\hat{\Th}_j}\label{eq:s_grad}
\end{align}
and
\begin{align}
	&\dpd{}{s_i}\left(\x_k-\v{A}\v{g}(\x_{k-1})\right)^\tr\v{Q}^{-1}\left(\x_k-\v{A}\v{g}(\x_{k-1})\right)\\
	=&-2\v{g}(\x_{k-1})^\tr\dpd{\v{A}}{s_i}^\tr\v{Q}^{-1}\left(\x_k-\v{A}\v{g}(\x_{k-1})\right)\\
	=&-2\Tr{\dpd{\v{A}}{s_i}^\tr\v{Q}^{-1}\x_k\v{g}(\x_{k-1})^\tr}-2\Tr{\dpd{\v{A}}{s_i}^\tr\v{Q}^{-1}\v{A}\v{g}(\x_{k-1})\v{g}(\x_{k-1})^\tr}\\
	=&-2\Tr{\dpd{\v{A}}{s_i}^\tr\v{Q}^{-1}\left(\x_k\v{g}(\x_{k-1})^\tr-\v{A}\v{g}(\x_{k-1})\v{g}(\x_{k-1})^\tr\right)}\label{eq:a_grad}.
\end{align}
Combining equations \eqref{eq:a_grad} and \eqref{eq:s_grad} then gives
\begin{multline}
	\dpd{\Lb(\Th,\hat{\Th}_j)}{s_i}=\\
	-2\Tr{\dpd{\v{A}}{s_i}^\tr\v{Q}^{-1}\left(\sum_{k=1}^T\E{\x_k\v{g}(\x_{k-1})^\tr}-\v{A}\sum_{k=1}^T\E{\v{g}(\x_{k-1})\v{g}(\x_{k-1})^\tr}\right)}
	\label{eq:pdcLH_nonlinear}
\end{multline}
If we then set \eqref{eq:pdcLH_nonlinear} to zero, we have
\begin{align}
	\v{A}_{j+1}=\left(\sum_{k=1}^T\E{\x_k\v{g}(\x_{k-1})^\tr}\right)\left(\sum_{k=1}^T\E{\v{g}(\x_{k-1})\v{g}(\x_{k-1})^\tr}\right)^{-1}
\end{align}
...
\begin{split}
	...
	\left(\v{B}_1-\v{A}\v{B}_2^\tr-\v{B}_2\v{A}^\tr+\v{A}\v{B}_3\v{A}^\tr\right)}\\
	&+\Tr{\v{Q}^{-1}\left(-\dpd{\v{A}}{\theta_i}\v{B}_2^\tr-\v{B}_2\dpd{\v{A}}{\theta_i}^\tr+\dpd{\v{A}}{\theta_i}\v{B}_3\v{A}^\tr+\v{A}\v{B}_3\dpd{\v{A}}{\theta_i}^\tr\right)}\\
	&+\Tr{-\v{R}^{-1}\dpd{\v{R}}{\theta_i}\v{R}^{-1}
	\left(\v{B}_4-\v{H}\v{B}_5^\tr-\v{B}_5\v{H}^\tr+\v{H}\v{B}_1\v{H}^\tr\right)}\\
	&+T\Tr{\v{Q}^{-1}\dpd{\v{Q}}{\theta_i}}
	+T\Tr{\v{R}^{-1}\dpd{\v{R}}{\theta_i}}\\
	\label{eq:dLB}
\end{split}
\end{align}
...
\begin{align}
	...
	\frac{1}{2}\Tr{\v{R}^{-1}\sum_{k=1}^T\E{\dpd{}{t_i}\left(\y_k-\v{H}\v{l}(\x_k)\right)\left(\y_k-\v{H}\v{l}(\x_k)\right)^\tr}{\hat{\Th}_j}}
\label{eq:s_grad}
\end{align}
and
...
If we then set \eqref{eq:pdcLH_nonlinear} to zero, we have
\begin{align}
	\v{H}_{j+1}=\left(\sum_{k=1}^T\y_k\E{\v{l}(\x_{k})^\tr}\right)\left(\sum_{k=1}^T\E{\v{l}(\x_{k})\v{l}(\x_{k})^\tr}\right)^{-1}
\end{align}
...
\begin{split}
	-2\dpd{I_2}{\theta_i}
	=&-\Tr{\v{Q}^{-1}\dpd{\v{Q}}{\theta_i}\v{Q}^{-1}
	\sum_{k=1}^T\E{\left(\x_k-\v{f}(\x_{k-1})\right)\left(\x_k-\v{f}(\x_{k-1})\right)^\tr}{\hat{\Th}_j}}\\
	&-\Tr{\v{Q}^{-1}\sum_{k=1}^T\E{\dpd{\v{f}(\x_{k-1})}{\theta_i}\left(\x_k-\v{f}(\x_{k-1})\right)^\tr+\left(\x_k-\v{f}(\x_{k-1})\right)\dpd{\v{f}^\tr(\x_{k-1})}{\theta_i}}{\hat{\Th}_j}}\\
	&+T\Tr{\v{Q}^{-1}\dpd{\v{Q}}{\theta_i}}\\
	\dpd{I_3}{\theta_i}=&
	-\Tr{\v{R}^{-1}\dpd{\v{R}}{\theta_i}\v{R}^{-1}
	\sum_{k=1}^T\E{\left(\y_k-\v{h}(\x_{k})\right)\left(\y_k-\v{h}(\x_{k})\right)^\tr}{\hat{\Th}_j}}\\
	&-\Tr{\v{R}^{-1}\sum_{k=1}^T\E{\dpd{\v{h}(\x_{k})}{\theta_i}\left(\y_k-\v{h}(\x_{k})\right)^\tr+\left(\y_k-\v{h}(\x_{k})\right)\dpd{\v{h}^\tr(\x_{k})}{\theta_i}}{\hat{\Th}_j}}\\
	&+T\Tr{\v{R}^{-1}\dpd{\v{R}}{\theta_i}}\\
