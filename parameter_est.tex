...
for a point $\x_\star$ to be a local maximum. We can define a local maximum
to be a point $\x_\star$, for which $\lLH[\Th]-\lLH[\Th+\v{d}] \geq 0$ for small $\v{d}$.

\begin{proposition}
Let $\ell:\Theta \to \R$ have continuous second order partial derivatives and let $\x_\star$
...
\end{enumerate}
then $\x_\_star$ is a strict local maximum of $\ell$.
\end{proposition}
 \begin{proof}
 Taylor expanding around $\Th+\v{d}$ gives
 \begin{align}
	\lLH[\Th]-\lLH[\Th+\v{d}] &= \frac{1}{2}\v{d}^\tr\nabla^2\lLH[\Th_\star]\v{d}-\F{o}{\abs{\v{d}}^2}\\
	&\leq 0,\;\mathrm{when }\v{d}\to 0
\end{align}
 \end{proof}
...
The simplest gradient based method is probably the \emph{method of steepest descent}.
It requires that the first partial derivatives of the objective function are defined
and continuous in their domain. The method of steepest descent is then defined
by the iteration
\begin{align}
	\Th_{j+1}&=\Th_j+\alpha_j\nabla\lLH[\Th_j].
	\label{eq:steepest_descent}
\end{align}
Intuitively, the method moves in the direction
of the negative gradient (thus $\alpha_j < 0$ when minimizing) and
the \emph{step-size} is determined by $\alpha_j$.
To determine $\alpha_j$, another minimization problem needs to be solved,
namely
\begin{align}
...
to solve the step-sizes are known as \emph{line searches} \parencite{luenberger2008}.
