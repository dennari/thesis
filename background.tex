...
\begin{align}
	\xk &= \v{f}_{\Th}\left(\x_{k-1}\right)+\v{q}_{k-1}\\
	\yk &= \v{h}_{\Th}\left(\xk\right)+\v{r}_{k}\\
	\v{q}_k&\sim \N{\v{0},\v{Q}}\\
	\v{r}_k&\sim \N{\v{0},\v{R}}\\
	\v{x}_{0} &\sim \N{\gv{\mu},\gv{\Sigma}}
	\label{eq:ssm_nonlinear}
\end{align}
\end{subequations}
Then clearly $\v{f}_{\Th}:\mathcal{X}\to\mathcal{X}$ and
