...
\begin{figure}[!htp]
	...
	\tikzstyle{tconnect}=[-latex, thin, opacity=0.25]
	  ...
	  \node[main,node distance=7mm] (theta) [right of=x__1,above of=next,label=above:$\Th$] {};
	  \path
	    ...
	    (theta) edge [tconnect] (x__1)
	    	...
	    	edge [tconnect] (y__1);
	\end{tikzpicture}
	\caption{SSM as a graphical model presented with a directed acyclic graph. All the other nodes
	in the graph are conditioned on the parameter $\Th$.}
