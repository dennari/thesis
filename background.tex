...
$\f_k$ and $\h_k$ and the noise processes $\v{q}$ and $\v{r}$ will be stationary (i.e. independent of
k). Thus the SSMs considered in this thesis are of the form
\begin{subequations}
\label{eq:ssm_general}
\begin{alignat}{2}
	\xk &= \ff+\v{q}_{k-1} \qquad& \v{q}_{k-1}&\sim \N{\v{0}}{\v{Q}} \label{eq:Q}\\
	\yk &= \hh+\v{r}_{k} & \v{r}_k&\sim \N{\v{0}}{\v{R}} \label{eq:R}\\
...
\Leftrightarrow \Pdf{\x}&=\N[\x]{\m}{\P}\\
	&\equiv \fparen*{\fparen*{2\pi}^{d_x}\detr\P}^{-\sfrac{1}{2}}\exp\fparen[\big]{-\frac{1}{2}(\x-\m)^\tr\P^{-1}(\x-\m)}\\		
...
\begin{align}
	\Pdf{\xk}{\x_{k-1},\Th}&=\N{\ff}{\v{Q}}\label{eq:dynamics_distr_general}\\
	\Pdf{\yk}{\xk,\Th}&=\N{\hh}{\v{R}}\label{eq:measurement_distr_general}
\end{align}
\end{subequations}
